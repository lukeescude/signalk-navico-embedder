...
- **Reworked the MFD Apps config panel into separate Enabled and Disabled lists.** Apps now render in two labeled, count-badged drop zones instead of one list with per-row checkboxes: drag an app between the lists to enable/disable it, or drag within a list to reorder. Enabling drops an app at the bottom of the Enabled group and disabling drops it at the top of the Disabled group, so the underlying `apps` array stays ordered `[...enabled, ...disabled]` — which is also the order enabled apps are announced to the MFD. Larger (48px) icons, and the per-app "Description" input was dropped from the rows
- **Icons of already-listed apps are now refreshed on webapp discovery.** Previously discovery only appended newly-found apps; existing entries kept their stale saved icon path. Discovery now re-reads each known app's icon from the discovered set, so an app that changed or gained an icon shows the current one
- Added `/skServer/loginStatus` to the proxy's always-allowed path prefixes so apps can check login status through the proxy
- Documentation: marked [caveman-chartplotter](https://www.npmjs.com/package/caveman-chartplotter), [signalk-maintenance-tracker](https://www.npmjs.com/package/signalk-maintenance-tracker), and [signalk-stowage-mgmt](https://www.npmjs.com/package/signalk-stowage-mgmt) as verified-working apps, added `caveman-chartplotter` to the recommended-plugins list, and refreshed the README's SignalK Tides support notes
